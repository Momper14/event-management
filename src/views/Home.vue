<template>
  <div class="home">
              ...
              <v-row :align="end" justify="end" style=" padding: 70px"> 
                 <form style="background-color: white; width:400px">
                   <v-container fluid style=" padding: 30px">
                    ...
                    <v-select v-model="select" :items="dates" label="Any Date" required>

                    </v-select>
                    <v-text-field v-model="location" label="In"
                    ></v-text-field>
                    <p>And I'm in the mood for</p>
                    <v-select v-model="category" :items="items" label="Anything"
                    ></v-select>
      ...
      <v-container style="padding-left: 5%">
        ...
        <v-list-item>
          <v-list-item-avatar color="grey">{{event.dateS}} <br> {{event.stime}}</v-list-item-avatar> 
          <v-list-item-content>
...
export default {
  ...
  methods: {
     ...
     deleteEvent(id){
       ...
       this.$store.dispatch('deletEvent', id)
     },
     editEvent(event){
       //eslint-disable-next-line
